...
def main():
    ...
    count_df["wikidata"] = count_df["prefix"].map(
        bioregistry.get_registry_map("wikidata.entity")
    )
    dd = dict(count_df[count_df["wikidata"].notna()][["wikidata", "count"]].values)
    print(dd)

    depdendency_df = pd.read_csv(
        DATA.joinpath("depdends_on_bioregistry.tsv"), sep="\t", dtype=str
    )
    for item, label in tqdm(depdendency_df[["item", "itemLabel"]].values):
        if item not in dd:
            try:
                count = pubmed_downloader.count_search_results(
                    '"' + label + '"', retmax=10
                )
            except Exception:
    ...
    depdendency_df = depdendency_df[
        depdendency_df["frequency"].notna() & (depdendency_df["frequency"] > 0)
    ]
